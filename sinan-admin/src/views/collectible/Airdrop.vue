  ...
  <div v-loading="loading" class="page-container">
    <div class="sn-card page-head">
      <div class="head-left">
        ...
        <div>
          ...
          <span class="head-sub">自库存池扣减发放，计入空投计数与流通量（文档 8.6 #44）</span>
        </div>
      </div>
    ...
    <template v-if="detail">
      <el-alert
        ...
        :closable="false"
        show-icon
        title="草稿藏品不可空投，请先完成发售配置"
        style="margin-bottom: 12px"
      />

      <!-- 空投表单 -->
      <div class="sn-card">
        <div class="card-title">空投发放（每用户份数 × 用户数 ≤ 库存池）</div>
        <el-form ref="formRef" :model="form" :rules="rules" label-width="130px" style="max-width: 640px">
          <el-form-item label="每用户份数" prop="quantity">
            <el-input-number
              ...
              style="width: 200px"
            />
            <div class="form-hint">当前库存池：<b class="din">{{ detail.stockPool }}</b>，发放总量超出将被拦截</div>
          </el-form-item>
          <el-form-item label="接收用户手机号" prop="phones">
            ...
            <div class="form-hint">共 <b class="din">{{ phoneLines.length }}</b> 个手机号 · 预计发放
              <b class="din">{{ phoneLines.length * form.quantity }}</b> 份</div>
          </el-form-item>
          <el-form-item>
            <el-button
              v-permission="'collectible:airdrop'"
              type="primary"
              :loading="submitting"
    ...
    <PasswordVerify
      ...
      hint="空投将自库存池扣减并直接发放给用户（计入流通量），需管理员密码确认。"
    />
  </div>
</template>

<script setup lang="ts">
// 独立空投（文档 8.6 #44 + 11.1：二次弹窗 + 密码）+ 空投记录（#49）
import { computed, onMounted, reactive, ref } from 'vue'
import { useRoute, useRouter } from 'vue-router'
import { ElMessage } from 'element-plus'
import type { FormInstance, FormRules } from 'element-plus'
import PasswordVerify from '@/components/PasswordVerify.vue'
import { fetchCollectibleDetail, airdropCollectible, fetchAirdropRecords } from '@/api/collectible'
import type { AirdropRecordRow, AirdropResult, CollectibleDetail, PageData } from '@/types/api'
...
async function submit(): Promise<void> {
  ...
  try {
    result.value = await airdropCollectible(id, {
...
async function loadRecords(): Promise<void> {
  ...
  try {
    const data = (await fetchAirdropRecords(id, { page: page.value, pageSize: pageSize.value })) as PageData<AirdropRecordRow>
