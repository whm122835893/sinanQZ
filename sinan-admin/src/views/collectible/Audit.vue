  ...
  <div v-loading="loading" class="page-container">
    <div class="sn-card page-head">
      <div class="head-left">
        ...
        <div>
          <h3 class="head-title">库存审计</h3>
          <span class="head-sub">{{ detail?.name || '—' }} · 守恒校验（文档 4.3.1 / 8.6 #48）</span>
        </div>
      </div>
    ...
    <template v-if="audit">
      ...
      <div class="sn-card">
        <div class="card-title-row">
          <span class="card-title">① 库存守恒校验</span>
      ...
      <div class="sn-card">
        ...
        <el-descriptions :column="3" border size="small" style="margin-bottom: 12px">
          ...
          <el-descriptions-item label="藏品 reserved_count">{{ audit.counters.reservedCount }}</el-descriptions-item>
        </el-descriptions>
...
// 库存审计（文档 8.6 #48：守恒校验，4.3.1 公式）
import { computed, onMounted, ref } from 'vue'
import { useRoute, useRouter } from 'vue-router'
import { fetchCollectibleDetail, fetchCollectibleAudit } from '@/api/collectible'
import type { CollectibleDetail, InventoryAuditResult } from '@/types/api'
...
const audit = ref<InventoryAuditResult | null>(null)
...
async function load(): Promise<void> {
  loading.value = true
  try {
    const [d, a] = await Promise.all([fetchCollectibleDetail(id), fetchCollectibleAudit(id)])
    detail.value = d
    audit.value = a
  } catch {
    // 拦截器已提示
