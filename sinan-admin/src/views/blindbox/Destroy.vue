  ...
  <div v-loading="loading" class="page-container">
    <div class="sn-card page-head">
      <div class="head-left">
        ...
        <div>
          <h3 class="head-title">销毁盲盒库存</h3>
          <span class="head-sub">不可逆操作：自盲盒库存池扣减并写入销毁记录与审计（文档 8.7 #61）</span>
        </div>
      </div>
    ...
    <template v-if="detail">
      <el-alert
        ...
        show-icon
        title="销毁不可逆"
        description="销毁数量自盲盒库存池永久扣减，销毁后无法恢复，请谨慎操作；每次操作均需管理员密码并写审计日志。"
        style="margin-bottom: 12px"
      />

      <!-- 销毁表单 -->
      <div class="sn-card">
        <div class="card-title">销毁盲盒库存（仅发售中/已售罄可销毁，文档 6.2）</div>
        <el-form
          ref="formRef"
          :model="form"
          :rules="rules"
        ...
        >
          <el-form-item label="销毁数量" prop="quantity">
            <el-input-number
              ...
              style="width: 200px"
            />
            <div class="form-hint">当前盲盒库存池：<b class="din">{{ detail.stockPool }}</b>，销毁数量不可超过库存池</div>
          </el-form-item>
    ...
    <PasswordVerify
      ...
      hint="销毁为不可逆操作，将自盲盒库存池永久扣减指定数量，需管理员密码确认。"
    />
  </div>
</template>

<script setup lang="ts">
// 销毁盲盒库存（文档 8.7 #61 + 11.1：二次弹窗 + 数量 + 密码）+ 销毁记录（#67）
import { computed, onMounted, reactive, ref } from 'vue'
import { useRoute, useRouter } from 'vue-router'
import { ElMessage } from 'element-plus'
import type { FormInstance, FormRules } from 'element-plus'
import PasswordVerify from '@/components/PasswordVerify.vue'
import { fetchBlindboxDetail, destroyBlindbox, fetchBlindboxDestroyRecords } from '@/api/blindbox'
import type { BlindboxDetail, DestroyRecordRow, PageData } from '@/types/api'
...
/** 销毁仅 onsale/soldout 状态可用（文档 6.2 状态机） */
...
async function submit(): Promise<void> {
  ...
  const ok = await pwdRef.value?.open({ title: '销毁盲盒库存确认', requireReason: false })
  if (!ok) return

  submitting.value = true
  try {
    result.value = await destroyBlindbox(id, {
...
async function loadRecords(): Promise<void> {
  ...
  try {
    const data = (await fetchBlindboxDestroyRecords(id, { page: page.value, pageSize: pageSize.value })) as PageData<DestroyRecordRow>
