...
// 图形验证码（场景 auth_forgot：发送找回密码短信前置）
const captcha = useCaptcha('auth_forgot')
...
async function sendCode() {
  ...
  try {
    // 图形码前置（场景 auth_forgot）
    const payload = captcha.inject({ phone: phone.value, scene: 'reset_password' })
