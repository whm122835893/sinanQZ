-- ============================================================================
-- sinanQZ 开发联调种子数据（seed-dev.sql）—— 仅供开发环境，生产环境严禁执行！
-- 用途：为「抽奖 / 社区 / 邀请」模块的前后端联调提供最小业务数据。
-- 警告：本脚本会写入虚构藏品/用户/资金等业务假数据，若在生产库执行将污染线上
--       业务数据，且无法通过「幂等」回滚。部署/运维前请再次确认目标库为开发库。
-- 说明：init.sql 按设计规范不写入任何业务 Mock 数据，本脚本仅用于
--       开发/联调环境，生产环境请勿执行。
-- 特性：固定主键 + ON DUPLICATE KEY UPDATE，幂等可重复执行。
-- 用法：mysql -uroot -p < database/dev-only/seed-dev.sql
-- ============================================================================
