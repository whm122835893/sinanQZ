...
class CollectibleController extends BaseController
{
    ...
    /**
     * POST /admin/collectibles/swap-preview
     * 统一置换预览（只读）：按源藏品比例计算受影响用户名单与空投份数
     *
     * 参数：
     *   old_collectibles   [{collectible_id, ratio}, ...] 源（被回收）藏品及比例
     *   new_collectible_id 目标（新）藏品ID
     */
    public function swapPreview()
    {
        $itemsRaw = $this->request->param('old_collectibles', []);
        $newId    = $this->request->param('new_collectible_id');
        $reason   = trim((string) $this->request->param('reason', ''));

        try {
            $params = \app\admin\service\SwapPlanService::parseParams($itemsRaw, $newId, $reason);
            $preview = \app\admin\service\SwapPlanService::preview($params['items'], $params['newId']);
        } catch (\app\service\InventoryException $e) {
            return $this->fail(4220, $e->getMessage());
        } catch (\Throwable $e) {
            return $this->fail(5000, '置换预览失败：' . $e->getMessage());
        }
     ...
     *
     * 参数：
     *   old_collectibles   [{collectible_id, ratio}, ...] 源（被回收）藏品及比例（每持有 1 份 → 空投 ratio 份）
     *   new_collectible_id  目标（新）藏品ID
     *   reason              置换原因（审计）
     *
     * 精准控制要点：
     *   1. 回收与空投共用同一次行锁 SELECT 的用户列表（单一数据源）
     *   2. 全程单事务，任一步失败全部回滚
     *   3. 生成置换计划：源配置 / 用户名单 / 资产明细全程留痕
     */
    public function swap()
    {
        ...
        try {
            ...
            return $this->fail(5000, '置换参数错误：' . $e->getMessage());
        }

        $operator = ['id' => $this->adminId(), 'name' => $this->adminName(), 'ip' => (string) $this->request->ip()];

        try {
            $result = \app\admin\service\SwapPlanService::execute(
                $params['items'],
                $params['newId'],
                $params['reason'],
                $operator
        ...
        $this->audit('collectible', 'swap',
            '统一置换：回收 ' . count($params['items']) . ' 个源藏品共 ' . $result['recoveredCount'] . ' 份（'
            . $result['userCount'] . ' 人）→ 按比例空投新藏品 ' . $result['airdropQuantity'] . ' 份',
            [
                'plan_no'            => $result['planNo'],
                'old_collectibles'   => $params['items'],
                'new_collectible_id' => $params['newId'],
                'recovered_count'    => $result['recoveredCount'],
                'user_count'         => $result['userCount'],
                'airdrop_quantity'   => $result['airdropQuantity'],
                'reason'             => $params['reason'],
            ],
            'collectible', $params['newId']
        );

        return $this->success($result, '置换完成：已统一回收源藏品并按比例向持有人空投新藏品');
    }
